...
## ADR-010: Dynamic Multi-Provider LLM Routing (placeholder)

> Note: the dynamic-provider routing plan was produced during plan-mode
> sessions in `docs/PLAN_cost_optimization.md`. ADR-010 is the *first*
> ADR to be accepted on that path; it lives here as a placeholder to
> preserve numbering once the corresponding code lands. The full text
> will be added by the IMPLEMENTER session that lands the router.
